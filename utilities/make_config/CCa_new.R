...
df_C <- readRDS(paste(filepath, "df_C_new", sep = ""))
rC_N3 <- as.numeric(df_C[df_C$atom == "O2", 3:5])
Cxyz <- df_C[, 3:5]
...
df_CCa <- rbind(df_C_new, df_Ca)

saveRDS(df_CCa, file = paste(filepath, "df_CCa", sep = ""))
write_to_gro(paste(filepath, "CCa.gro", sep = ""), df_CCa)
