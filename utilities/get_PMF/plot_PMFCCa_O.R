...
full <- read.table(file = "ang_base_pair_top/CCa_O/umbrella/profile.xvg" )
full_av <- 
  full %>%
  filter((V1 > 1.) & (V1 < 1.05)) %>%
...
GT <- read.table(file = "ang_base_pair_top/CCa_O_GT/umbrella/profile.xvg" )
GT_av <- 
  GT %>%
  filter((V1 > 1.) & (V1 < 1.05)) %>%
  summarise(bulk = mean(V2)) %>%
