...
library(tidyverse)
library(pracma)
df_NO3Mg <- readRDS("df_NO3Mg")
a <- read_table(file = "NO3Mgcharges.txt", col_names = FALSE)
a$X1 == df_NO3Mg$atom
...
proA_coord_charge <- cbind(df_NO3[, 4:7])
colnames(proA_coord_charge) <- c("x", "y", "z", "q")

proB_coord_charge <- cbind(df_Mg[, 4:7])
...
# need the Ewald summation to see the real effects.